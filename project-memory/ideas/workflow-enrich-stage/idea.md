...
**Reason deferred:** Deferred from the V1 PRD so V1 can focus on the core `extract → link → validate → write graph` workflow for `../ecommerce-demo-app`.
...
Add `enrich` as a first-class Rivière workflow stage in future product work.

## Why this may matter

The user confirmed this is not rejected work and should not fall off the roadmap.
