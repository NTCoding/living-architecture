...
describe('GraphContext', () => {
  ...
  describe('URL loading', () => {
    ...
    it('loads graph from URL successfully', async () => {
      ...
      function TestUrlConsumer(): React.ReactElement {
        const { loadGraphFromUrl, hasGraph, isLoadingFromUrl, urlLoadError } = useGraph()
        return (
          <div>
            <button onClick={() => void loadGraphFromUrl('http://example.com/graph.json')}>
              Load
            </button>
            <span data-testid="loading">{isLoadingFromUrl ? 'loading' : 'done'}</span>
            <span data-testid="has-graph">{hasGraph ? 'yes' : 'no'}</span>
            <span data-testid="error">{urlLoadError ?? 'none'}</span>
          </div>
        )
      }
    ...
    it('handles HTTP error (404)', async () => {
      ...
      function TestUrlConsumer(): React.ReactElement {
        const { loadGraphFromUrl, urlLoadError } = useGraph()
        return (
          <div>
            <button onClick={() => void loadGraphFromUrl('http://example.com/missing.json')}>
              Load
            </button>
            <span data-testid="error">{urlLoadError ?? 'none'}</span>
          </div>
    ...
    it('handles invalid JSON', async () => {
      ...
      function TestUrlConsumer(): React.ReactElement {
        ...
        return (
          <div>
            <button onClick={() => void loadGraphFromUrl('http://example.com/bad.json')}>
              Load
            </button>
            <span data-testid="error">{urlLoadError ?? 'none'}</span>
          </div>
    ...
    it('clearUrlLoadError clears error state', async () => {
      vi.stubGlobal('fetch', vi.fn().mockResolvedValue({
        ok: false,
        status: 500,
      }))
      ...
      function TestUrlConsumer(): React.ReactElement {
        const { loadGraphFromUrl, clearUrlLoadError, urlLoadError } = useGraph()
        return (
          <div>
            <button onClick={() => void loadGraphFromUrl('http://example.com/graph.json')}>
              Load
            </button>
            <button onClick={() => clearUrlLoadError()}>Clear Error</button>
            <span data-testid="error">{urlLoadError ?? 'none'}</span>
          </div>
        )
      }
