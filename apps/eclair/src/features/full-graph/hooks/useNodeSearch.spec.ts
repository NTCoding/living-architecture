...
describe('filterNodesBySearch', () => {
  ...
  it('returns all node IDs when query is whitespace only', () => {
    const result = filterNodesBySearch('   ', testNodes, testEdges)
  ...
  it('matches nodes by name case-insensitively', () => {
  ...
  it('includes connected nodes in visible set', () => {
  ...
  it('returns empty sets when no nodes match', () => {
  ...
  it('handles multiple matching nodes and merges their connections', () => {
    const result = filterNodesBySearch('API', testNodes, testEdges)
