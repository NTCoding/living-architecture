...
const renderCodeLink = (sourceLocation: {
  filePath: string
  lineNumber: number
  repository: string
}): React.ReactNode => (
  <span data-testid="code-link-path">{sourceLocation.filePath}:{sourceLocation.lineNumber}</span>
)
...
function createEntity(
  ...
  return {
    ...
    domain: overrides.domain ?? 'orders',
    operations,
    states: overrides.states ?? ['Draft', 'Pending', 'Confirmed', 'Cancelled'],
    transitions: [],
...
describe('EntityAccordion', () => {
  ...
  describe('code links', () => {
    it('renders code link menu for operations with sourceLocation', async () => {
      ...
      render(<EntityAccordion entity={entity} renderCodeLink={renderCodeLink} />)
