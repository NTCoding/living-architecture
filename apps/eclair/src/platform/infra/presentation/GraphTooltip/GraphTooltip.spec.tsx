...
import type { GraphTooltipData } from './GraphTooltip'

vi.mock('@/platform/infra/presentation/CodeLinkMenu/CodeLinkMenu', () => ({
  CodeLinkMenu: ({
    filePath,
    lineNumber,
  }: {
    ...
    <span data-testid="code-link-path">{filePath}:{lineNumber}</span>
  ),
}))

const mockTooltipData: GraphTooltipData = {
  domain: 'orders',
  incomingCount: 3,
  name: 'Test API Node',
  outgoingCount: 5,
...
describe('GraphTooltip', () => {
  ...
  it('uses singular "edge" for count of 1', () => {
    const dataWithOneEdge: GraphTooltipData = {
      ...mockTooltipData,
      incomingCount: 1,
      outgoingCount: 1,
    }
  ...
  it('shows code link when node has sourceLocation', () => {
    const dataWithSource: GraphTooltipData = {
      ...mockTooltipData,
      sourceLocation: {
  ...
  describe('viewport edge detection', () => {
    ...
    it('repositions tooltip to the left when near right viewport edge', () => {
      const nearRightEdge: GraphTooltipData = {
        ...mockTooltipData,
        x: 900,
        y: 200,
      }
    ...
    it('repositions tooltip above when near bottom viewport edge', () => {
      const nearBottomEdge: GraphTooltipData = {
        ...mockTooltipData,
        x: 100,
        y: 700,
      }
    ...
    it('keeps tooltip in viewport when near corner', () => {
      const nearCorner: GraphTooltipData = {
        ...mockTooltipData,
        x: 900,
        y: 700,
      }
    ...
    it('positions normally when not near edges', () => {
      const safePosition: GraphTooltipData = {
        ...mockTooltipData,
        x: 100,
        y: 200,
      }
