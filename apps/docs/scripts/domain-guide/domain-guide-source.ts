...
export function inspectSubdomains(workspaceRoot: string): readonly SubdomainGuideEntry[] {
  const packagesRoot = path.join(workspaceRoot, 'packages')
  const cliSources = readProductionSources(path.join(workspaceRoot, 'apps/cli/src'))
...
function inspectSubdomain(
  ...
  const descriptionManifest = domainModelManifest ?? publishedLanguageManifest
  if (descriptionManifest === undefined) {
    throw new DomainGuideSourceError(`Subdomain '${subdomain}' has no domain package.`)
  }
