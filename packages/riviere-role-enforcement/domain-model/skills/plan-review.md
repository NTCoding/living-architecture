...
6. **Respect import rules.** If the plan proposes an import outside a location's inherited or explicitly allowed `importRules.allow`, flag it immediately.
...
1. **`.riviere/role-enforcement.config.ts`** — learn:
   ...
   - `locations` — each location's allowed roles, sublocations, and import rules
   - `ignorePatterns`
...
- **Wrong-location code.** The plan proposes an import not permitted by the source location's import rules.
