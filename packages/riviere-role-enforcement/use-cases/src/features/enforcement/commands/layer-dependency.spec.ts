...
const layerTestLocations = enforcementBuilder.locationConfiguration(
  enforcementBuilder.location<(typeof layerTestRoles)[number]['name']>('/domain', ['domain-port']),
  enforcementBuilder.location<(typeof layerTestRoles)[number]['name']>('/infra', ['role-infra'], {
    importRules: { allow: {} },
  }),
)
...
const layerTestBootstrap = {
  ...
  roles: layerTestRoles,
  files: {
    'packages/pkg-a/src/infra/source.ts': `/** @riviere-role role-infra */
...
it('accepts imports within the infra layer', () => {
  fixtureWorkspace.withWorkspaceFixture(layerTestBootstrap, (workspaceDir) => {
    fixtureWorkspace.writeFixtureFile(
      workspaceDir,
      'packages/pkg-a/src/infra/consumer.ts',
...
function createAdapterLayerConfig() {
  const locations = enforcementBuilder.locationConfiguration(
    enforcementBuilder.location<(typeof layerTestRoles)[number]['name']>(
      '/adapters/{adapter}',
      ['domain-port-adapter'],
      {
        ...
        importRules: {
          allow: {
            root: [{ domain: ['domain-port'] }, { infra: ['external-client-service'] }],
          },
        },
      },
    ),
    enforcementBuilder.location<(typeof layerTestRoles)[number]['name']>('/commands', [
      'command-use-case',
    ]),
    enforcementBuilder.location<(typeof layerTestRoles)[number]['name']>(
      '/domain',
    ...
    enforcementBuilder.location<(typeof layerTestRoles)[number]['name']>('/infra', [
