...
function buildTopLevelRulesModule(parsed: TopLevelRulesConfig): Module {
  return {
    name: 'extended',
    path: '.',
    glob: '**',
...
function parseConfigContent(content: string, source: string): Module {
  ...
  if (isTopLevelRulesConfig(parsed)) {
    return buildTopLevelRulesModule(parsed)
  }
...
export class ExtractionConfigRepository {
  resolveSourceFiles(resolvedConfig: ResolvedExtractionConfig, configDir: string): string[] {
  ...
  load(configPath: string): ValidatedConfig {
