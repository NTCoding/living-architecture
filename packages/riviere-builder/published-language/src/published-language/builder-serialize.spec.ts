...
describe('RiviereBuilder', () => {
  ...
  describe('resume', () => {
    ...
    it('preserves components from serialized graph', () => {
      ...
      expect(
        resumed.build().components.filter((component) => component.type === 'API'),
      ).toHaveLength(1)
    })
    ...
    it('allows continued building after resume', () => {
      ...
      expect(resumed.build().components).toHaveLength(2)
      expect(
        resumed.build().components.filter((component) => component.type === 'API'),
      ).toHaveLength(2)
    })
    ...
    it('preserves custom types from serialized graph', () => {
      ...
      expect(resumed.build().components).toHaveLength(1)
      expect(
        resumed.build().components.filter((component) => component.type === 'Custom'),
      ).toHaveLength(1)
    })
    ...
    it('allows continued linking after resume', () => {
      ...
      expect(resumed.build().links).toHaveLength(2)
      expect(resumed.build().components).toHaveLength(3)
    })
