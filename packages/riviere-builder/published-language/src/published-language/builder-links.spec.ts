...
describe('RiviereBuilder', () => {
  describe('link', () => {
    ...
    it('reports an invalid source component ID without attempting suggestions', () => {
      ...
      expect(() =>
        builder.link({
          from: 'not-a-component-id',
          to: 'any:target:id',
        }),
      ).toThrow("Source component 'not-a-component-id' not found")
    })
  ...
  describe('linkExternal', () => {
    it('creates external link when source exists', () => {
      ...
      const { link: externalLink } = builder.linkExternal({
        from: source.id,
        target: {
          name: 'Stripe API',
          domain: 'payments',
    ...
    it('deduplicates duplicate external links and records DUPLICATE_LINK_SKIPPED warning', () => {
      ...
      const duplicate = builder.linkExternal({
        from: source.id,
        target: {
          repository: 'acme/payments',
          name: 'Stripe API',
      ...
      expect(builder.build().externalLinks).toHaveLength(1)
    ...
    it('records a duplicate external target without a repository', () => {
      ...
      const duplicate = builder.linkExternal(input)
