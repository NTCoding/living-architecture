...
describe('RiviereBuilder upsert warnings', () => {
  it('emits SCALAR_OVERWRITE warning for changed scalar fields only', () => {
    ...
    const overwriteWarnings = result.warnings.filter(
      (warning) => warning.code === 'SCALAR_OVERWRITE',
    )
  ...
  it('does not emit SCALAR_OVERWRITE for unchanged value or noOverwrite skip', () => {
    ...
    const unchanged = builder.upsertUseCase({
      name: 'Place Order',
      domain: 'orders',
      module: 'checkout',
      description: 'same',
    ...
    const preserved = builder.upsertUseCase(
      {
        name: 'Place Order',
        domain: 'orders',
        module: 'checkout',
    ...
    expect(unchanged.warnings).toStrictEqual([])
    expect(preserved.warnings).toStrictEqual([])
  })
