...
describe('Component', () => {
  it('rejects API, UseCase, and DomainOp updates whose type differs', () => {
    expect(() => Component.create(ui(api().id)).update(api(), preference)).toThrow(
      ComponentTypeMismatchError,
    )
    expect(() => Component.create(ui(useCase().id)).update(useCase(), preference)).toThrow(
      ComponentTypeMismatchError,
    )
    expect(() =>
      Component.create(ui(domainOperation().id)).update(domainOperation(), preference),
    ).toThrow(ComponentTypeMismatchError)
  })

  it('rejects Event, EventHandler, and Custom updates whose type differs', () => {
    expect(() => Component.create(ui(event().id)).update(event(), preference)).toThrow(
      ComponentTypeMismatchError,
    )
    expect(() =>
      Component.create(ui(eventHandler().id)).update(eventHandler(), preference),
    ).toThrow(ComponentTypeMismatchError)
    expect(() => Component.create(ui(custom().id)).update(custom(), preference)).toThrow(
      ComponentTypeMismatchError,
    )
  })

  it('keeps absent optional values absent during updates', () => {
    expect(Component.create(api()).update(api(), preference).component).toStrictEqual(api())
    expect(
      Component.create(domainOperation()).update(domainOperation(), preference).component,
    ).toStrictEqual(domainOperation())
    expect(Component.create(event()).update(event(), preference).component).toStrictEqual(event())
    expect(Component.create(custom()).update(custom(), preference).component).toStrictEqual(
      custom(),
    )
  })
  ...
  it('keeps supplied optional values during updates', () => {
    ...
    expect(
      Component.create(apiWithOptionalValues).update(apiWithOptionalValues, preference).component,
    ).toStrictEqual(apiWithOptionalValues)
    expect(
      Component.create(domainOperationWithOptionalValues).update(
        domainOperationWithOptionalValues,
        preference,
      ).component,
    ).toStrictEqual(domainOperationWithOptionalValues)
    expect(
      Component.create(eventWithSchema).update(eventWithSchema, preference).component,
    ).toStrictEqual(eventWithSchema)
    expect(
      Component.create(customWithDescription).update(customWithDescription, preference).component,
