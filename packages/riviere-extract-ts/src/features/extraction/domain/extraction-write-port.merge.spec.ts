...
import { DuplicateComponentError } from '@living-architecture/riviere-builder'
import {
  mergeWritePort,
...
describe('mergeWritePort', () => {
  ...
  it('swallows duplicate component errors from custom type definition and continues writing', () => {
    const workflowBuilder = createWorkflowBuilder()
    vi.spyOn(workflowBuilder, 'defineCustomType').mockImplementation(() => {
      throw new DuplicateComponentError('backgroundJob')
    })
    const diagnostics: WorkflowDiagnostics = { report: vi.fn() }
    const writePort = mergeWritePort(workflowBuilder, diagnostics, {
      step: 'extract-orders',
  ...
  it('rethrows non-duplicate errors from custom type definition', () => {
    const workflowBuilder = createWorkflowBuilder()
    vi.spyOn(workflowBuilder, 'defineCustomType').mockImplementation(() => {
    ...
    expect(() => writePort.addComponent(createCustomInput())).toThrow(
      SameStepDuplicateComponentError,
    )
  })
