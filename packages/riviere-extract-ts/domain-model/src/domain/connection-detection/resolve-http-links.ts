...
export function resolveHttpLinks(
  ...
  for (const link of links) {
    ...
    resolvedLinks.push(
      ExtractedLink.parse({
        source: link.source,
        target: ComponentId.parseFromParts(matchedApi).toString(),
...
export function stripResolvedCustomTypes(
  ...
  return components.filter((component) => {
    ...
    return targetedIdentities.has(ComponentId.parseFromParts(component).toString())
  })
}
...
function indexComponentsByIdentity(
  ...
  for (const component of components) {
    byIdentity.set(ComponentId.parseFromParts(component).toString(), component)
  }
  return byIdentity
}
