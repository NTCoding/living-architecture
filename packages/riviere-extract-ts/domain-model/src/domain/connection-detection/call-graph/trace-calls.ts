...
function traceCallExpression(callExpr: CallExpression, ctx: TraceContext): void {
  ...
  if (outcome.target !== undefined) {
    if (
      ComponentId.parseFromParts(ctx.sourceComponent).toString() !==
      ComponentId.parseFromParts(outcome.target).toString()
    ) {
      ctx.results.push(
        RawLink.parse({
...
function traceIntoNonComponent(
  ...
  const { method: resolvedMethod, classFound } = findMethodInProject(
    ctx.project,
    traceTypeName,
    calledMethodName,
  )
