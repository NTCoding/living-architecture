...
describe('evaluatePredicate', () => {
  ...
  describe('hasJSDoc', () => {
    it('returns true when method has matching JSDoc tag', () => {
      ...
      expect(
        evaluatePredicate(sf.getClassOrThrow('S').getMethodOrThrow('process'), {
          hasJSDoc: { tag: 'riviere' },
        }),
      ).toBe(true)
    })
  ...
  describe('extendsClass', () => {
    it('returns true when class extends specified base class', () => {
      ...
      expect(
        evaluatePredicate(sf.getClassOrThrow('OrderCreated'), {
          extendsClass: { name: 'DomainEvent' },
        }),
      ).toBe(true)
    })
  ...
  describe('implementsInterface', () => {
    it('returns true when class implements specified interface', () => {
      ...
      expect(
        evaluatePredicate(sf.getClassOrThrow('OrderHandler'), {
          implementsInterface: { name: 'IHandler' },
        }),
      ).toBe(true)
    })
    ...
    it('returns false when node is not a class', () => {
      const sf = createTestProject(`function fn() {}`)
      expect(
        evaluatePredicate(sf.getFunctionOrThrow('fn'), {
          implementsInterface: { name: 'IHandler' },
        }),
      ).toBe(false)
    })
