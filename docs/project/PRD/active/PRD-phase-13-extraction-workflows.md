...
| 45  | `riviere-workflow` package follows monorepo repository hygiene: ADR-002 folder structure and dependency rules added to role enforcement, role tags on every export, 100% test coverage, workspace-reference imports only, and explicit adapter isolation so only `platform/infra/external-clients` may import `@eventcatalog/sdk`, `@asyncapi/parser`, or Node `child_process`                                                                                                                      | Role enforcement, lint, and coverage gates green on CI, including location rules that forbid vendor SDK/parser/process imports outside generic external clients                                                                                                                                                                                                                     |
...
- **D1.5:** Workflow schema and package foundations are strict by default
  ...
  - Acceptance criteria: schema tests cover structural validation rules; resolver tests prove file-relative path behaviour; role-enforcement, coverage, and workspace-import gates apply to the new package; vendor SDK/parser/process imports are confined to `platform/infra/external-clients`.
