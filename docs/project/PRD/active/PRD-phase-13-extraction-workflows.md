...
**Aggregate-repository loading-method rule:** Every public aggregate-loading method on an `aggregate-repository` is named exactly `load` or begins with `loadBy`. The role check enforces this naming contract before any remaining Phase 13 implementation begins. Names such as `loadWorkflow`, `loadForEnrichment`, and `loadFromPersistedState` fail; names such as `load`, `loadById`, `loadByWorkflowPath`, and `loadByEnrichment` pass. Optional persistence methods such as `save` and private repository assembly helpers are outside this loading-method naming constraint.
...
6. Per-stage ts-morph resources are disposed before the stage completes, so multiple `code-extraction` stages do not retain compiler state.
...
- **Aggregate-repository loading methods:** public methods that load and return an aggregate are named `load` or `loadBy*`. Role enforcement rejects other loading names without imposing that pattern on optional persistence methods or private helpers.
...
- R0.8 Transition fixtures exist for every completed stage and are generated through the documented capture procedure (§3.8.3), never edited by hand.
...
| 7   | `tests/workflow-transitions/NN-after-<stage>.json`                   | Does not exist                                                       | New per-stage fixtures generated through the documented capture procedure (§3.8.3).                                                                                                                    |
...
**Fixture generation procedure (mandatory for R0.8):** Fixtures are generated by instrumenting a known-good workflow run with a test-only observer that captures the readonly Builder component, Link, and external-Link values returned after each completed stage, writing to `tests/workflow-transitions/NN-after-<stage-name>.json`. The observer belongs to `ecommerce-demo-app` tooling and consumes returned stage-transition values; production domain code has no fixture hook. Regenerating a fixture requires a known-good run (the author confirms the graph state is correct), then committing the generated JSON — never editing fixtures by hand.
...
- Fixtures serialise the stage-transition values produced from `builder.components()`, `builder.links()`, and `builder.externalLinks()` through the test observer; the published readonly Builder surface (§3.5.1) is authoritative.
...
| 26  | ts-morph `Project` instances created during `code-extraction` are disposed before stage completion; multiple extraction stages do not retain compiler state                                                                                                                                                                                                                                                                                                                                                                    | Memory-pressure test runs five extraction stages sequentially and asserts bounded retained heap                                                                                                                                                                                                              |
...
| 33  | AI stages (`ai-extract`, `ai-enrich`) invoke the configured CLI executable through the `RiviereProject` AI CLI domain port and its use-case adapter over `child_process.spawn` (`shell: false`), pass the prompt via stdin by default or a single `{prompt}` argv placeholder, capture stdout, validate it against the response schema in `riviere-extract-config`, enforce `timeout-seconds`, and apply results through typed `upsert*` with `{ noOverwrite: true }`. Riviere imports no AI SDK and reads no API-key env vars | Unit tests for the port adapter with a mocked generic process client; integration test with a tiny stub CLI script; dependency assertions confirm no AI SDK; timeout test kills the child process                                                                                                            |
...
| 42  | Per-stage transition fixtures are generated through the documented test observer by serialising readonly Builder values after each completed stage; fixtures are never hand-edited                                                                                                                                                                                                                                                                                                                                             | Demo-app tooling and CI prove known-good fixture regeneration is byte-stable                                                                                                                                                                                                                                 |
...
| 49  | Role enforcement rejects public aggregate-loading methods whose names are neither `load` nor prefixed `loadBy`, while accepting `load`, `loadBy*`, optional persistence methods such as `save`, and private assembly helpers                                                                                                                                                                                                                                                                                                   | Role-enforcement domain/plugin tests cover accepted `load`, `loadById`, `loadByWorkflowPath`, and `loadByEnrichment` methods plus rejected `loadWorkflow`, `loadForEnrichment`, and `loadFromPersistedState` methods; the repository-wide role check passes after current violations are renamed or replaced |
...
| ----------------------------------------------- | ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Aggregate-repository loading-method enforcement | D1.0                   | Add the approved role constraint that permits public aggregate-loading methods named `load` or `loadBy*` and rejects every other loading-method name. Make the repository-wide role check pass before any other remaining Phase 13 issue begins.                                   |
...
| Direct/workflow extraction parity               | D1.3                   | Reuse the same Project extraction behaviour, preserve same-stage duplicate failure, convert incomplete extraction state into Workflow diagnostics, and dispose TypeScript compiler resources after each stage.                                                                     |
| Strict Phase 13 workflow language               | D1.5                   | Replace the unpublished fixed-stage dialect with `apiVersion: v1`, file-addressed workflow execution, strict typed stage configs, file-relative path materialisation, shared schema references, and exhaustive parser output. Do not support two unrelated formats both called V1. |
...
| Shared AI CLI boundary                          | D3.1                   | Add the AI CLI domain port, isolated process client and adapter, command/args/timeout handling, strict response schemas, and stub integration tests without an AI SDK or credential surface.                                                                                       |
...
| Executable demo workflow and fixtures           | D0.3                   | After M1-M4 exist, add the root workflow, stage configs, ground truth, generated transition fixtures, capture tooling, and README workflow journey in `ecommerce-demo-app`.                                                                                                        |
...
1. D1.0 is created and delivered first. It blocks every other remaining Phase 13 issue.
2. After D1.0, D0.2 can run in parallel with the remaining M1 foundation.
3. D1.1, D1.2, and D1.5 gate importer, AI, and CLI execution work; D1.3 gates code-extraction parity in the executable demo.
4. D2 and D3 stage capabilities may proceed independently once their M1 dependencies are complete.
5. D4.1 requires the Project-owned stage model and strict workflow language; D4.2 requires the strict workflow language and validation path.
6. D0.3 executable fixtures begin only after the required M1-M4 capabilities exist. Static source/spec groundwork does not wait for those capabilities.
7. D5 begins only after the executable demo workflow and pinned-revision contract are complete.
...
- **D0.3:** Demo workflow and fixtures are reproducible
  ...
  - Verification: after the required M1-M4 capabilities land, run `workflow validate`, run the fixture-capture tooling, and assert a clean fixture diff in CI.
...
- **D1.0:** Aggregate-repository loading-method names are role-enforced
  ...
  - Acceptance criteria: role configuration expresses the approved rule; role-enforcement domain and plugin tests cover accepted `load`, `loadById`, `loadByWorkflowPath`, and `loadByEnrichment` names plus rejected `loadWorkflow`, `loadForEnrichment`, and `loadFromPersistedState` names; current repository violations are renamed or replaced so the repository-wide role check passes.
...
- **D1.3:** `code-extraction` reuses `RiviereProject` extraction behaviour
  ...
  - Verification: parity tests, same-stage duplicate tests, multiple-configuration Workflow tests, and resource-disposal tests for repeated extraction stages.
