...
**Workflow idempotency:** Running the same workflow twice with unchanged inputs must produce identical output under deterministic execution conditions. This always applies to deterministic-only workflows. For workflows with AI steps, it applies only when the runtime provides deterministic AI execution controls (for example: pinned model/version, deterministic inference settings, and replayable prompt inputs). Validated by running twice and diffing under those conditions.
...
| 13  | Workflow runs are idempotent under deterministic execution conditions (deterministic-only steps, or AI runs with deterministic runtime controls)                            | E2E test: run twice, diff outputs, assert zero changes under those conditions                                             |
