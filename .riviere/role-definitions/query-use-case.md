...
- **Not a command-use-case**: commands orchestrate write operations that may modify and save state. If nothing is modified or saved, use query-use-case.
- **Not a domain-service**: domain services contain pure business logic. If it loads a query model from persistence, it is a query-use-case.
- **Not a cli-entrypoint**: entrypoints translate external input into query-use-case-input and call the query. They do not load query models.
...
- **vs domain-service**: Does it load a query model from persistence? → query-use-case. Pure logic on passed-in data? → domain-service
